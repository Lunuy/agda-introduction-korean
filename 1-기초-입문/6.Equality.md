...
```
그리고 equivalence relation을 위와 같이 정의한다. 이 정의가 뜻하는 것은, 어떤 두 유리수 a, b에 대해서, common 이라는 유리수 하나가 존재해서, 이 common의 분모와 분자에 적절한 0이 아닌 자연수를 각각 곱함으로서 a와 b를 각각 만들어낼 수 있다면 이 두 유리수는 같다는 것이다. 기약분수의 개념을 생각해보면, 이 정의는 두 유리수의 같음을 잘 나타냄을 알 수 있다.
